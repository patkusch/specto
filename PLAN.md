...
| Speaker labels | whisperX (BSD) + pyannote (MIT code, gated model) vs. sherpa-onnx (Apache-2.0, ungated) | **Shipped in v0.8.0 as sherpa-onnx, not pyannote/whisperX.** whisperX's own diarization still goes through pyannote's gated model, so it needs the same Hugging Face account and token. sherpa-onnx runs pyannote's `segmentation-3.0` model re-exported to ONNX, plus a speaker-embedding model, both downloaded from a public GitHub release with no account or token. `pip install specto[speakers]`, `--speakers`. Measured on a 3-voice test clip: 6/6 turns, 5/5 speaker changes correct (see CHANGELOG 0.8.0 and `specto/diarize.py`); not yet measured on a real call. |
