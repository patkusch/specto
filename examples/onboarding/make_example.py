...
1. Fills the six page templates in `templates/` with the sample customer from
   `sample_customer()` (every name, date, number and street on the screens is
   built from a fixed seed, so nothing on them is anyone's real details) and
   writes the finished pages to `app/`.
2. Renders each page in `app/` to a 1280x720 PNG. Playwright (headless
   Chromium) is tried first; if it is not installed or cannot start, the same
   screens are drawn with Pillow instead.
3. Speaks each line of the narration with the Mac's `say` command and measures
...
4. Lays the lines out on a timeline, screen by screen, with a short pause after
...
# Every name, date of birth, phone number, street and reference number on the
# six screens comes from sample_customer() below, built from a fixed seed. The
# values are made to look like placeholders, so nobody mistakes them for a real
# person: the names are generated by gluing a start syllable to an end syllable
# (they are meant to look name-like and belong to nobody), dates of birth are
# always the 1st of January, phone numbers sit in the range Ofcom reserves for
# fiction, emails are at example.com and streets are named after test data.
SEED = 20260914
FIRST_STARTS = ["Mar", "Tal", "Ren", "Ves", "Cor", "Elo", "Nev", "Sor", "Wil", "Ode"]
FIRST_ENDS = ["isol", "wyn", "ette", "dra", "ric", "ina", "an", "ley", "ith", "ora"]
SURNAME_STARTS = ["Quen", "Vor", "Fal", "Wex", "Tam", "Ost", "Brin", "Cray", "Lun", "Dor"]
SURNAME_ENDS = ["by", "sley", "thorne", "wick", "stow", "combe", "shaw", "mere", "dale", "hurst"]
STREETS = ["Example Street", "Sample Row", "Placeholder Lane", "Fixture Road", "Specimen Avenue"]
POSTCODE = "SW1A 1AA"  # the canonical example postcode; it is spoken in the narration, so it stays fixed


def sample_customer(seed: int = SEED) -> dict:
    """The sample customer and everyone else named on the screens, from one seed.

    Same seed, same dict, every time. Change SEED (or pass another seed) and
    rebuild to get a different, equally fictional, set of people.
    """
    rng = random.Random(seed)
    firsts = rng.sample([a + b for a in FIRST_STARTS for b in FIRST_ENDS], 12)
    surnames = rng.sample([a + b for a in SURNAME_STARTS for b in SURNAME_ENDS], 8)
    first, last = firsts[0], surnames[0]

    def birth_date() -> str:
        # 1 January makes it visibly a placeholder while still being a valid date.
        return f"01/01/{rng.randint(1960, 1999)}"

    reference = rng.randint(10000, 99999)
    return {
        "first_name": first,
        "last_name": last,
        "date_of_birth": birth_date(),
        # 07700 900000 to 900999 is the range Ofcom reserves for drama and fiction.
        "phone": f"07700 900{rng.randint(0, 999):03d}",
        "email": f"{first.lower()}.{last.lower()}@example.com",
        "address_line_1": f"{rng.randint(1, 99)} {rng.choice(STREETS)}",
        "address_line_2": "",
        "town": "London",
        "postcode": POSTCODE,
        "reference": f"NW-2026-{reference:05d}",
        "signed_in_user": f"{firsts[1]} {surnames[1]}",   # the onboarding officer doing the walkthrough
        "team_lead": f"{firsts[2]} {surnames[2]}",        # approves, and is assigned the queue
        "colleague": f"{firsts[3]} {surnames[3]}",        # another officer who submitted customers
        # Customer Search results: people with the searched surname who already exist.
        "matches": [
            {"id": f"C-{rng.randint(100000, 999999)}", "name": f"{firsts[4 + i]} {last}", "date_of_birth": birth_date()}
            for i in range(3)
        ],
        # The other customers in the Approval Queue, in the order shown.
        "queue": [
            {"reference": f"NW-2026-{reference - offset:05d}", "name": f"{firsts[7 + i]} {surnames[4 + i]}"}
            for i, offset in enumerate((5, 2, 8))
        ],
    }


def page_values(customer: dict) -> dict[str, str]:
    """Flatten the customer into the {{placeholder}} names used by the templates."""
    values = {k: v for k, v in customer.items() if isinstance(v, str)}
    values["name"] = f"{customer['first_name']} {customer['last_name']}"
    values["address"] = ", ".join(
        part for part in (customer["address_line_1"], customer["address_line_2"], customer["town"], customer["postcode"]) if part
    )
    values["id_file"] = f"passport_{customer['first_name'][0].lower()}{customer['last_name'].lower()}.jpg"
    for i, match in enumerate(customer["matches"], 1):
        for key, value in match.items():
            values[f"match{i}_{key}"] = value
    for i, row in enumerate(customer["queue"], 1):
        for key, value in row.items():
            values[f"queue{i}_{key}"] = value
    return values
...
def render_pages(customer: dict | None = None) -> dict[str, str]:
    """Fill every template with the sample values and write the pages to app/.

    Returns {file name: rendered html}. A placeholder with no value is an error,
    so a typo in a template cannot leave `{{...}}` on a screen.
    """
    values = page_values(customer or sample_customer())
    rendered = {}
    for template in sorted(TEMPLATES.glob("*.html")):
        page = _PLACEHOLDER.sub(lambda m: html.escape(values[m.group(1)]), template.read_text(encoding="utf-8"))
        (APP / template.name).write_text(page, encoding="utf-8")
        rendered[template.name] = page
    return rendered
...
def render_with_pillow() -> None:
    ...
    content = {
        ...
        "review.html": ([], ["Name", c["name"]], "Submit for approval", None),
        ...
        "confirmation.html": ([], [], "Start another customer", f"Customer reference number  {c['reference']}"),
    }
    values = {"First name*": c["first_name"], "Last name*": c["last_name"], "Date of birth*": c["date_of_birth"], "Email address": c["email"],
              "Phone number": c["phone"], "Account type*": "Personal", "Address line 1": c["address_line_1"], "Address line 2": c["address_line_2"],
              "Town / City": c["town"], "Postcode*": c["postcode"], "Postcode": c["postcode"], "Surname": c["last_name"], "Document type*": "Passport", "ID document*": "Choose file"}
    for index, (name, html, _) in enumerate(SCRIPT):
        ...
        d.text((980, 18), f"Signed in as {c['signed_in_user']} - Onboarding Officer", fill="white", font=font(13))
...
def main() -> int:
    ...
    pages = render_pages(customer)
    print(f"pages: {len(pages)} written to {APP} for {customer['first_name']} {customer['last_name']} (seed {SEED})")
