...
@pytest.mark.parametrize("phone", ["07000 12345678", "+44 7700 345678", "+44 (0)20 7946 0958", "020-7946-0958"])
def test_phone_forms(phone):
    assert "phone" in kinds(f"Phone number {phone}")


def test_phone_near_misses():
    assert "phone" not in kinds("Product code PC0700012345678X")   # letters glued on: a product code
...
def test_date_of_birth_needs_a_birth_word():
    assert masked("Date of birth * 04/03/1975", "date of birth") == ["04/**/**75"]
    assert "date of birth" in kinds("DOB 1975-03-04")
    assert "date of birth" in kinds("she was born on the 4th of March 1975")
    assert "date of birth" not in kinds("Signed on 09/11/2020")
    assert "date of birth" not in kinds("Application date 23/11/1979")
...
def test_address():
    assert masked("48 Greek Street\nLondon", "address") == ["4* G*** S***"]
    assert "address" in kinds("Address line 1  221B Baker Street")
    assert "address" not in kinds("Step 48 Greek Street")   # mid-sentence, no label
    assert "address" not in kinds("48 greek street")        # not capitalised
...
def test_masking_rules():
    ...
    assert mask_value("phone", "07000 12345678") == "07*** ******78"
...
def test_hit_never_carries_the_raw_value():
    text = "Email priya.shah@example.com Phone 07000 12345678"
...
def test_scan_recording_sources_and_frames(recording, analysis):
    ...
    assert ("phone", "07*** ******78") in {(h.kind, h.value_masked) for h in frame_hits}
...
def test_pii_summary_wording():
    ...
    hits = [
        ...
        hit("phone", "07*** ******78", 3), hit("phone", "07*** ****24", 4),
